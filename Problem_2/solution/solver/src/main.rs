...
fn find_last_start_intersection(solution: &CauchySolution<6>) -> f64 {
    let mut res: f64 = 0.0;
    for i in 1..(solution.t.len() - 1) {
        ...
        if solution.x[i][1].atan2(solution.x[i][0]).abs() * R < 1000.0 {
            res = solution.t[i];
        }
    }
    res
...
fn main() {
    ...
    //         0.0,
    //         0.0,
    //         velocity * orbit_angle.cos(),
    //         velocity * orbit_angle.sin(),
    //     ],
    // };
    ...
    fn equation(x: &[f64; 1]) -> [f64; 1] {
        ...
        let x = *solution.x.last().unwrap();

        // [x[1].powf(2.0) + x[2].powf(2.0)]
        // [x[1].abs() + x[2].abs()]
        [x[1].atan2(x[0]).powf(2.0) + (x[2] / R).powf(2.0)]
    }

    let r_0 = R + 500000.0;

    let [r] = solvers::solve_newton(equation, &[r_0], None, true).unwrap();
    ...
    println!("equation(solution): {:?}", equation(&[r]));
